...
internal static class BridgeImage
{
    ...
    private const string Entrypoint = """
        ...
        : "${DOTNETDOC_HOST:=host.docker.internal}"
        : "${DOTNETDOC_PORT:=15640}"

        ip=$(getent hosts "$DOTNETDOC_HOST" | awk '{print $1}' | head -1)
        : "${ip:=$DOTNETDOC_HOST}"

        mkdir -p /srv/docs

        echo "dotnetdocfs-bridge: mounting 9p from $ip:$DOTNETDOC_PORT"
    ...
    internal static async Task<CommandResult> BuildAsync(CancellationToken cancellationToken = default)
    {
        ...
        string context = Directory.CreateTempSubdirectory("dotnetdocfs-bridge-").FullName;
