...
namespace DotnetDocFs.Internal.Mount;
...
internal static class Mounter
{
    ...
    internal static async Task MountAsync(MountSettings settings, CancellationToken cancellationToken = default)
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            throw new MountException(
                "Windows cannot mount this tree directly. Run dotnetdoc inside WSL and mount there:\n"
                + $"  sudo mount -t 9p -o trans=tcp,port={settings.NinePPort},version=9p2000.L 127.0.0.1 /mnt/dotnetdocfs\n"
                + "Windows then reads it at \\\\wsl$\\<distro>\\mnt\\dotnetdocfs.");
        }
    ...
    internal static async Task UnmountAsync(MountSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        if (mounted is null)
        {
            Console.WriteLine($"{settings.MountPath} is not a dotnetdocfs mount; nothing to unmount");
    ...
    private static async Task VerifyAsync(MountSettings settings, CancellationToken cancellationToken)
    {
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            Console.Error.WriteLine(
                ...
                + "within 15 seconds; the mount may be wedged. 'dotnetdoc --unmount' clears it up.");
        }
