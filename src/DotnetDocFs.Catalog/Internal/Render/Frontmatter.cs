...
/// <c>tags</c>, <c>timestamp</c> — is OKF's recommended set, and the fields this catalog adds of
/// its own (<c>assembly</c>, <c>namespace</c>, <c>framework</c>) are the producer-defined extras
...
internal sealed class Frontmatter
{
    ...
    /// <summary>
    /// Adds the ISO 8601 timestamp OKF asks for: the time the body of documentation this page
    /// belongs to was taken from the machine. Nothing here was authored at a point in time, and a
    /// clock read during rendering would answer whenever this page happened to be read first and
    /// then stand still, which is worse than no field at all.
    /// </summary>
    ...
    /// <summary>
    /// Quotes a scalar when YAML would otherwise read it as something else. Type names bring
    /// colons, braces and leading symbols into these fields, and an unquoted <c>{</c> starts a
    /// flow mapping.
    /// </summary>
    private static string Scalar(string value)
    {
        ...
        foreach (char c in flat)
        {
            quoted.Append(c switch
            {
                ...
                // A control character is not legal as itself even inside quotes, and XML
