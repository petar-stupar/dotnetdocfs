...
{
    [Fact]
    public void TheFlagParses() => Assert.True(CliOptions.Parse(["--version"]).Version);

    [Fact]
    public void TheDefaultIsNotToPrintAVersion() => Assert.False(CliOptions.Parse([]).Version);
    ...
    public void EveryComponentIsNamedWithARealVersion()
    {
        ...
        Assert.Equal(["dotnetdoc", "NineP.Server", "NineP.Protocol"], lines.Select(line => line.Split(' ')[0]));
