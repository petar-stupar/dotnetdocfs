...
public class MountGuardTests
{
    private static readonly MountSettings Settings =
        new("/home/me/mnt/dotnetdocfs", MountStrategy.Native, NinePPort: 15640, SmbPort: 14450);
    ...
    public void ADirect9PMountOfOursIsRecognised() =>
        Assert.Equal(
            ...
            HostMount.Identify(
                [
                    "127.0.0.1 on /home/me/mnt/dotnetdocfs type 9p "
                        + "(rw,relatime,sync,dirsync,access=any,trans=tcp,port=15640)",
                ],
                Settings));
    ...
    public void TheBridgesShareIsRecognisedOnLinux() =>
        Assert.Equal(
            ...
            HostMount.Identify(
                ["//127.0.0.1/dotnetdocs on /home/me/mnt/dotnetdocfs type cifs (rw,relatime,vers=3.1.1)"],
                Settings));
    ...
    public void TheBridgesShareIsRecognisedOnMacOs() =>
        Assert.Equal(
            MountStrategy.Docker,
            HostMount.Identify(
                ["//guest@127.0.0.1:14450/dotnetdocs on /home/me/mnt/dotnetdocfs (smbfs, nodev, nosuid, read-only)"],
                Settings));
    ...
    public void A9PMountOnSomeOtherPortIsNotOurs() =>
        Assert.Null(HostMount.Identify(
            ["127.0.0.1 on /home/me/mnt/dotnetdocfs type 9p (rw,trans=tcp,port=564)"],
            Settings));

    [Fact]
    public void AnOrdinaryFilesystemAtThePathIsNotOurs() =>
        Assert.Null(HostMount.Identify(
            ["/dev/nvme0n1p2 on /home/me/mnt/dotnetdocfs type ext4 (rw,relatime)"],
            Settings));
    ...
    public void AMountPointWithASpaceIsRecognised() =>
        Assert.Equal(
            MountStrategy.Native,
            HostMount.Identify(
                ["127.0.0.1 on /home/me/My\\040Documents/docs type 9p (rw,trans=tcp,port=15640)"],
    ...
    public void ADeviceNamedLikeOurMountPointIsNotAMatch() =>
        Assert.Null(HostMount.Identify(
            ["/home/me/mnt/dotnetdocfs on /mnt/somewhere-else type ext4 (rw)"],
            Settings));
}
