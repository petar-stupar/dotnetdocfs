...
VERSION="${DOTNETDOC_VERSION:-latest}"
BIN_DIR="${DOTNETDOC_BIN_DIR:-$HOME/.local/bin}"
...
archive="dotnetdoc-$bare-$rid.tar.gz"
...
[ -f "$tmp/dotnetdoc" ] || fail "$archive does not contain dotnetdoc"

mkdir -p "$BIN_DIR"
install -m 755 "$tmp/dotnetdoc" "$BIN_DIR/dotnetdoc" 2>/dev/null \
    || { cp "$tmp/dotnetdoc" "$BIN_DIR/dotnetdoc" && chmod 755 "$BIN_DIR/dotnetdoc"; }
...
if [ "$os" = osx ] && command -v xattr >/dev/null 2>&1; then
    xattr -d com.apple.quarantine "$BIN_DIR/dotnetdoc" 2>/dev/null || true
fi

echo "install.sh: installed $tag to $BIN_DIR/dotnetdoc"

case ":$PATH:" in
    *":$BIN_DIR:"*)
        echo "install.sh: run 'dotnetdoc --help'" ;;
    *)
