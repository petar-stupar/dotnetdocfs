<#
.SYNOPSIS
    Install dotnetdoc from a GitHub release.
...
.PARAMETER BinDir
    Where to put the binary. %LOCALAPPDATA%\Programs\dotnetdoc by default.
#>
[CmdletBinding()]
param(
    [string] $Version = 'latest',
    [string] $BinDir = (Join-Path $env:LOCALAPPDATA 'Programs\dotnetdoc')
)

$ErrorActionPreference = 'Stop'
$repo = 'petar-stupar/dotnetdocfs'

$arch = switch ($env:PROCESSOR_ARCHITECTURE) {
    'AMD64' { 'x64' }
    'ARM64' { 'arm64' }
    default { throw "dotnetdoc has no build for $env:PROCESSOR_ARCHITECTURE" }
}
...
$archive = "dotnetdoc-$bare-win-$arch.zip"
...
try {
    ...
    $binary = Join-Path $tmp 'dotnetdoc.exe'
    if (-not (Test-Path $binary)) { throw "$archive does not contain dotnetdoc.exe" }

    New-Item -ItemType Directory -Path $BinDir -Force | Out-Null
    Copy-Item $binary (Join-Path $BinDir 'dotnetdoc.exe') -Force

    Write-Host "install.ps1: installed $tag to $(Join-Path $BinDir 'dotnetdoc.exe')"
    ...
    if ($userPath -split ';' -notcontains $BinDir) {
        ...
        Write-Host "install.ps1: run 'dotnetdoc --help'"
    }
}
finally {
